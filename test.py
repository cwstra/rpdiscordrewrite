...
def predictedChart(number_of_dice, dice_face, filename):
...
def actualChart(prefix, die_str, totals, frequency, filename, predhandle):
    plt.title(prefix+" recorded results for "+die_str)
    plt.xlabel("Dice Totals")
...
if __name__ == "__main__":
    ...
    beg = timer()
    predhandle = predictedChart(number_of_dice, dice_face, 'predwork.png')
    end = timer()
    print(end-beg)
    ...
    print(len(testCount))
    beg = timer()
    actualChart("Jim the Test Muffin's", str(number_of_dice)+'d'+str(dice_face), list(testCount.keys()), list(testCount.values()), 'actwork.png', predhandle)
    end = timer()
    print(end-beg)
